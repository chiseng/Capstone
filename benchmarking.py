...
class CUDA:
    def __init__(self, file_name, y1, y2, x1, x2, sub_ch):
        self.mask: cv2.cuda.createBackgroundSubtractorMOG2 = cv2.cuda.createBackgroundSubtractorMOG2(history=3, varThreshold=100, detectShadows=False)
        self.filter = cv2.cuda.createGaussianFilter(cv2.CV_8UC1, cv2.CV_8UC1, (blur_value, blur_value), 0.5)
    ...
    def cuda_run(self):
        ...
        return self.blur_bgsub, self.median_blur, self.bgsub, self.thresh, self.contour_detection, fps

class standard:
    def __init__(self, filename):
        self.mask: cv2.createBackgroundSubtractorMOG2 = cv2.createBackgroundSubtractorMOG2(history=3,
                                                                                      varThreshold=100,
                                                                                      detectShadows=False)
    ...
    def vips_to_array(vi):
        return np.ndarray(buffer=vi.write_to_memory(), dtype=format_to_dtype[vi.format],
    ...
    def standard_run(self, x1:int, x2:int, y1:int, y2:int, sub_ch: list):
        fps = FPS().start()
        start = time.time()
...
def main(cuda=False):
    ...
    if cuda:
        ...
        blur_bgsub, median_blur, bgsub, thresh, contour_detection, fps = run_cuda.cuda_run()
    else:
        run_standard = standard(file_name)
        blur_bgsub, median_blur, bgsub, thresh, contour_detection, fps = run_standard.standard_run(x1, x2, y1, y2, sub_ch)
